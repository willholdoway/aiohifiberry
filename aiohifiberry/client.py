"""Client for the HiFiBerry OS / HBOS NG AudioControl REST API."""
...
class AudioControlClient:
        ...
        for url in urls:
            try:
                return await asyncio.wait_for(
                    self._request_url(method, url, **kwargs),
    ...
    @staticmethod
    def clean_artist(artist: Any) -> str | None:
    ...
    @staticmethod
    def base64_urlsafe(value: str) -> str:
        """Encode strings the way the HiFiBerry Web UI cover-art loader does."""
        encoded = base64.b64encode(value.encode()).decode()
        return encoded.replace("+", "-").replace("/", "_").rstrip("=")

    @staticmethod
    def best_cover_art_url(response: dict[str, Any]) -> str | None:
        ...
        for result in response.get("results") or []:
            for image in result.get("images") or []:
                ...
                best_pixels = (best_image.get("width") or 0) * (
                    best_image.get("height") or 0
                )
    ...
    async def async_update_cover_art(self) -> None:
        ...
        artist = self.clean_artist(song.get("artist"))
        ...
        for lookup in lookups:
            path = "/".join(self.base64_urlsafe(part) for part in lookup[1:])
            try:
                ...
                continue
            cover_art_url = self.best_cover_art_url(response)
            if cover_art_url:
    ...
    def _active_player_name(self, command: str | None = None) -> str | None:
        ...
        if isinstance(player, dict):
            ...
            player_state = str(
                player.get("state") or self.now_playing.get("state") or ""
            ).lower()
            if (
                player_name
        ...
        active_players = [
            ...
            and str(player.get("state") or "").lower()
            not in ("unknown", "disconnected")
        ]
